...
public:
     ...
     */
    void emitExpression(goParser::ExpressionContext *ctx);

    /**
     * Emit code for a simple expression.
     * @param ctx the SimpleExpressionContext.
     */
    void emitSimpleExpression(goParser::SimpleExpressionContext *ctx);

    /**
     * Emit code for a term.
     * @param ctx the TermContext.
     */
    void emitTerm(goParser::TermContext *ctx);

    /**
     * Emit code for NOT.
     * @param ctx the NotFactorContext.
     */
    void emitNotFactor(goParser::NotFactorContext *ctx);
     ...
     */
    void emitLoadValue(goParser::VariableContext *varCtx);
     ...
     */
    Typespec *emitLoadVariable(goParser::VariableContext *varCtx);

    /**
     * Emit code to load an integer constant.
     * @parm intCtx the IntegerConstantContext.
     */
    void emitLoadIntegerConstant(goParser::NumberContext *intCtx);

    /**
     * Emit code to load real constant.
     * @parm intCtx the IntegerConstantContext.
     */
    void emitLoadRealConstant(goParser::NumberContext *realCtx);
